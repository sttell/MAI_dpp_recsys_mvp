...
inline void ThrowFailReason(const FailReason& reason) {
    switch (reason.code) {
        ...
        case FailReason::Code::NOT_FOUND:
            throw common::exceptions::ConfigNotFoundError(reason.description);
        case FailReason::Code::UNEXPECTED_FORMAT:
            throw common::exceptions::UnexpectedFileFormatError(reason.description);
        case FailReason::Code::UNEXPECTED_PATH:
            throw common::exceptions::WrongPathError(reason.description);
    }
}

namespace recsys_t2s::config::utils {
